...
let opts = {
  ...
  if (!data.dispatcher) play(client, ops, data);
    ...
    message.channel.send(`Added to queue: ${info.title} | Requester: ${message.author.id}`);
...
async function play(client, ops, data) {
  ...
  client.channels.get(data.queue[0].announceChannel).send(`Now Playing: ${data.queue[0].songTitle} | Requester: ${data.queue[0].requester}`);
