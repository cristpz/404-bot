...
if(cmd === `${prefix}help`){
  ...
  .setColor("#3a0be7")
  .setThumbnail(bicon)
  .addField("Command Count", "180 currently")
  ...
  .addField("Fun", " `8ball` `cat` `dog` `google` `pick` `rate` `ratewaifu` `neko` `generateavatar` `yn` `lizard` `gasmoji` `coinflip` `define` `motivate` `meme` `shitpost` `emoji` `mal` ")
  .addField("Music", " `join` `nowplaying` `queue` `queueadd` `remove` `skip` `volume` `play` `leave` ")
  ...
  .addField("Moderation", " `kick` `ban` `addrole` `removerole` `lockdown` ")
