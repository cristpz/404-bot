...
if(cmd === `${prefix}help`){
  ...
  .addField("Fun", " `8ball` `cat` `dog` `google` `pick` `rate` `ratewaifu` `neko` `generateavatar` `yn` `lizard` `gasmoji` `coinflip` `define` `motivate` `meme` `shitpost` `emoji` `mal` `roll` ")
  .addField("Music", " `join` `nowplaying` `queue` `queueadd` `remove` `skip` `volume` `play` `leave` `pause` `resume` ")
